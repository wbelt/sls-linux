...
{{ lgsm.app }} cron SHELL settings:
  cron.env_present:
    - user: {{ lgsm.os.user }}
    - name: SHELL
    - value: /bin/bash

{{ lgsm.app }} cron PATH settings:
  cron.env_present:
    - user: {{ lgsm.os.user }}
    - name: PATH
    - value: /usr/sbin:/usr/bin:/bin

{{ lgsm.app }}  cron monitor:
  cron.present:
